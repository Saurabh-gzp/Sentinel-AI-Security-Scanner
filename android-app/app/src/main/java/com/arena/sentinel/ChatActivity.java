...
public class ChatActivity extends Activity {
    ...
    private void sendQuestion() {
        ...
        input.setText(""); addBubble("You", question, true, true); addAction("Using current scan context — no rescan requested");
        cancelRequested = false; // new question: clear any previous stop request
        setBusy(true);
    ...
    private void setBusy(boolean on) {
        // NOTE: cancelRequested is intentionally NOT reset here. The stop button sets it to true and
        // relies on it staying true so in-flight stream callbacks ignore remaining chunks. It is
        // cleared only when a brand-new question starts (see sendQuestion).
        busy = on;
        send.setText(on ? "■" : "SEND");
        status.setText(on ? "Sentinel is thinking" : "Evidence-grounded chat");
        if (on) animateDots(); else if (dotsTask != null) handler.removeCallbacks(dotsTask);
    }
